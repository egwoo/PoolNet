            ...
            if test_mode == 0:
                ...
                for k in range(0, len(scale)):
                    ...
                    with torch.no_grad():
                        ...
                        pred_0 = np.squeeze(
                            torch.sigmoid(preds[1][0]).cpu().data.numpy()
                        )
                        pred_1 = np.squeeze(
                            torch.sigmoid(preds[1][1]).cpu().data.numpy()
                        )
                        pred_2 = np.squeeze(
                            torch.sigmoid(preds[1][2]).cpu().data.numpy()
                        )
                ...
                cv2.imwrite(
                    os.path.join(
                        self.config.test_fold,
                        name[:-4] + "_" + mode_name[test_mode] + ".png",
                    ),
                    multi_fuse,
                )
            elif test_mode == 1:
                with torch.no_grad():
                    ...
                    os.makedirs(
                        os.path.join(self.config.test_fold, path), exist_ok=True
                    )
                    cv2.imwrite(
                        os.path.join(
                            self.config.test_fold,
    ...
    def train(self):
        ...
        for epoch in range(self.config.epoch):
            ...
            if (epoch + 1) % self.config.epoch_save == 0:
                torch.save(
                    self.net.state_dict(),
                    "%s/models/epoch_%d.pth" % (self.config.save_folder, epoch + 1),
                )
        ...
        torch.save(
            self.net.state_dict(), "%s/models/final.pth" % self.config.save_folder
        )
...
def bce2d(input, target, reduction=None):
    ...
    return F.binary_cross_entropy_with_logits(
        input, target, weights, reduction=reduction
    )

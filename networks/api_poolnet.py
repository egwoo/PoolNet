...
class EdgeInfoLayerC(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        for i_x in x:
            tmp_x.append(
                F.interpolate(i_x, x_size[2:], mode="bilinear", align_corners=True)
            )
...
class FuseLayer1(nn.Module):
    ...
    def forward(self, list_x, x_size):
        ...
        for i, i_x in enumerate(list_x):
            up_x.append(
                F.interpolate(
                    self.trans[i](i_x), x_size[2:], mode="bilinear", align_corners=True
                )
            )
